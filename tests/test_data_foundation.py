...
from scripts.data_common import sha256_file, write_csv_rows, write_json
from scripts.generate_demo_data import generate_dataset
from scripts.seed_data import SeedError, _seed_version
from scripts.validate_data import _verify_scenarios, validate_dataset
    ...
    write_json(
        config_path,
        {
            ...
            "periods": {
                ...
                "price_month_count": 2,
                "reference_quarter_starts": ["2025-01-01"],
            },
...
def test_generator_outputs_validate_and_are_byte_deterministic(tmp_path):
    ...
    assert summary["generated_rows"]["crop_references.csv"] == 1
...
def test_canonical_dataset_has_complete_crop_and_geography_coverage():
    ...
    assert errors == []
    assert summary["active_crops"] == 35
    assert summary["regions"] == 8
    assert summary["provinces"] == 38
    assert summary["municipalities_cities"] == 771
    assert summary["generated_rows"]["crop_profiles.csv"] == 35
    assert summary["generated_rows"]["price_history.csv"] == 49_140
    assert summary["generated_rows"]["crop_references.csv"] == 107_940
    assert summary["generated_rows"]["supply_snapshots.csv"] == 107_940
    assert summary["generated_rows"]["soil_suitability.csv"] == 26_985
...
class ExistingDatasetCursor:
    def __init__(self, metadata, manifest_hash, row_counts):
    ...
    def execute(self, statement, _params=None):
        if "SELECT manifest_sha256, metadata FROM dataset_metadata" in statement:
            self.row = (self.manifest_hash, self.metadata)
            return
